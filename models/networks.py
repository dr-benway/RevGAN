...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect', output_tanh=True, n_downsampling = 2):
        assert(n_blocks >= 0)
...
class ResnetGenerator_enc_noise(nn.Module):
    def __init__(self, ngf, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect', coupling='additive'):
        assert(n_blocks >= 0)
...
class ResnetGenerator_enc(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect', n_downsampling = 2, coupling='additive'):
        assert(n_blocks >= 0)
...
class ResnetGenerator_dec(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect', output_tanh=True, n_downsampling=2, coupling='additive'):
        assert(n_blocks >= 0)
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        ...
        for n in range(1, n_layers):
            ...
            nf_mult = min(2**n, 8)
            sequence += [
                spectral_norm(nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias)), nn.LeakyReLU(0.2, True)
            ]
        nf_mult_prev = nf_mult
